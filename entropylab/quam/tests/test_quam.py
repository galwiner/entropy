...
from entropylab.quam.core import (
    QuamTransmon,
    QuamReadoutResonator,
    QuamController,
)
...
def test_resonator_spectroscopy_separated():
    ...
    def test_admin(admin):
        ...
        xmon.add(
            Operation(
                ControlPulse(
                    ...
                    [
                        ArbitraryWaveform(
                            "wf_ramp", list(np.linspace(-0.5, 0.5, 1000))
                        ),
                        zero_wf,
                    ],
                    1000,
        ...
        xmon.add(
            Operation(
                ControlPulse(
                    ...
                    [
                        ArbitraryWaveform(
                            "pi_wf_opt", admin.config_vars.parameter("pi_wf_samples")
                        ),
                        zero_wf,
                    ],
                    1000,
        ...
        ror.mixer = Mixer(
            ...
            correction=Matrix2x2([[1, 0], [0, 1]]),
        )  # TODO: add default correction matrix
        ro_pulse = MeasurePulse(
            "readout_pulse",
            [
        ...
        config = (
            admin.build_qua_config()
        )  # TODO: add a test that the built config is correct
        commit_id = admin.commit("set config vars")
        # print(commit_id)
        admin.params.checkout(
            commit_id
        )  # checking we can also checkout from the ParamStore
        return commit_id

    def test_oracle(oracle, c_id):
        # Run a resonator spectroscopy as a user
        oracle.load(c_id)
        assert set(oracle.element_names) == set(
            ["cont1", "xmon", "ror"]
        ...
        assert set(oracle.user_params) == set(
            [
                "pi_wf_samples",
                "ro_amp",
                "ro_duration",
    ...
    def test_quam(quam, c_id):
        ...
        c_id = quam.commit(
            label="update config vars"
        )  # user is able to commit to param store
        f_start = int(10e6)
        f_end = int(50e6)
        df = int(1e6)
